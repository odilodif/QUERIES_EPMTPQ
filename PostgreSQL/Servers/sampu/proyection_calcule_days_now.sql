...
_days=0;
SELECT
   (CASE WHEN DATE_PART('year', AGE(now(), '2011-09-13')) >0 THEN DATE_PART('year', AGE(now(), '2011-09-13')) * 360 ELSE 0 END) + (CASE WHEN DATE_PART('month', AGE(now(), '2011-09-13')) >0 THEN DATE_PART('month', AGE(now(), '2011-09-13')) * 60 ELSE 0 END) + ( DATE_PART('day', AGE(now(), '2011-09-13')))  AS total_days;
END;
$f$
...
SELECT  AGE('2021-09-01', '2019-02-02') as age_nomal;