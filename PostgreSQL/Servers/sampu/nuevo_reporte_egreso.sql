SELECT
    ...
    detalle.subtotal_inegd,
		...
		'33' AS stock,
	( SELECT  'CODIGO: ' || maint_work_order1.id || ' REFERENCIA: ' || maint_work_order1.reference_number || '  FECHA DE ORDEN DE TRABAJO: ' || maint_work_order1.start_date  FROM  maint_work_order maint_work_order1 WHERE maint_work_order1.id = ingreso.id_maint_work_order  ) AS aswork_order,
	 (SELECT 'CODIGO: ' || maint_cost_center1.code || ' CENTRO DE COSTOS: ' || maint_cost_center1.name  FROM maint_cost_center maint_cost_center1 WHERE maint_cost_center1.id = ingreso.id_maint_cost_center )  AS center_cost
