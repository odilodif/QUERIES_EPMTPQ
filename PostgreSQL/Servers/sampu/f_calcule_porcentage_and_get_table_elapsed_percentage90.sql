--DROP function f_calcule_porcentage_and_get_table_elapsed_percentage90(integer);
CREATE OR REPLACE FUNCTION f_calcule_porcentage_and_get_table_elapsed_percentage90 (crs_id  INT DEFAULT 0)
...
_query= 'SELECT * FROM detention_all'; 
	FOR _date_inq, _yearsq, _monthsq, _daysq, _prontuarioq, _name_pplq,_last_name_pplq, _number_dtq, _name_crsq,_crs_idq   IN EXECUTE _query LOOP
			...
			END LOOP;	
	RETURN QUERY	 
 SELECT * FROM porcentaje_cumplido WHERE porcent_cumplido BETWEEN 90 AND 99.99;
...
SELECT f_calcule_porcentage_and_get_table_elapsed_percentage90();