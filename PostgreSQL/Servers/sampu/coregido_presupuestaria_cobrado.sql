...
from
    (
		...
		select extract(month from mov.mov_fecha_comov) as periodo,
     ...
     from cont_movimiento mov /*Encabezado*/
     left join tes_comprobante_pago cp on cp.ide_tecpo=mov.ide_tecpo
     left join (select b.ide_tepro,
     ...
     left join
         (
                ...
                substring (APELLIDO_PATERNO_GTEMP || ' ' || (case when APELLIDO_MATERNO_GTEMP is null then ''  else APELLIDO_MATERNO_GTEMP  end) || ' ' || PRIMER_NOMBRE_GTEMP || ' ' || (case when SEGUNDO_NOMBRE_GTEMP is null then '' else SEGUNDO_NOMBRE_GTEMP
                  end) from 1 for 30) AS NOMBRES,
		...
		/*Presupuestaria se combina con la 113.13 por el APP*/			
     join
         (				 
						 ...
						 select dmov.ide_comov,
							...
							left join
									(select ide_codem,
													...
													sum(cobrado_prmen) as devengado_prmen
					...
					) dmovi on dmovi.ide_comov=mov.ide_comov --dame las cuentas 113.13 con presupuestaria que tengan encabezado movimiento con detalle de mov y su presupuestaria
