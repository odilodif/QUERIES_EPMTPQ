    ...
    (case when coalesce(devengado_prmen,0)>0 then coalesce(dmovi3.cue_codigo_cocac,coalesce(dmovii.cue_codigo_cocac,'111.06.00')) else '623.01.00' end) as cta_mayor_gasto_i,
     ...
     left join
         (
          ...
          substring (APELLIDO_PATERNO_GTEMP || ' ' || (case when APELLIDO_MATERNO_GTEMP is null then '' else APELLIDO_MATERNO_GTEMP end) || ' ' || PRIMER_NOMBRE_GTEMP || ' ' || (case when SEGUNDO_NOMBRE_GTEMP is null then '' else SEGUNDO_NOMBRE_GTEMP end) from 1 for 30) AS NOMBRES,
         ...
         (select dmov.ide_comov,
          ...
          left join
              (select ide_codem,
               ...
               group by ide_codem,
                        pcl.codigo_clasificador_prcla) pmesi on pmesi.ide_codem=dmov.ide_codem
     ...
     left join
         (
          ...
          where coalesce(debe_codem,0)>0
					 ...
					 ) dmovi3 on dmovi3.ide_comov=mov.ide_comov
     and dmovi3.ide_gelua=dmovi.ide_gelua
     left join
         (
				 select dmov.ide_comov,
                 ide_gelua,
          ...
          from cont_detalle_movimiento dmov
          join cont_catalogo_cuenta cat on cat.ide_cocac=dmov.ide_cocac
          and (cue_codigo_cocac like '623%')
					
					) dmovii on dmovii.ide_comov=mov.ide_comov
