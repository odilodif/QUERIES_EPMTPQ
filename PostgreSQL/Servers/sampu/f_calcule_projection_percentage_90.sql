-- DROP FUNCTION f_calcule_projection_percentage_90(INTEGER , date)
CREATE OR REPLACE FUNCTION f_calcule_projection_percentage_90 (crs_id  INT DEFAULT 0, date_projection date DEFAULT '1900-01-01')  RETURNS SETOF  projection_percentage
	AS $BODY$
	DECLARE 
...
_query= 'SELECT * FROM detention_all'; 
	FOR _date_inq, _yearsq, _monthsq, _daysq, _prontuarioq,_nameppl,_last_nameppl, _number_dtq, _name_crsq,_crs_id   IN EXECUTE _query LOOP
			...
			END LOOP;	
	RETURN QUERY	 
 SELECT * FROM projection_percentage WHERE porcent_cumplido BETWEEN 80 AND 89.99;
...
select  f_calcule_projection_percentage_90(0,'2021-09-01');

select * from  f_calcule_projection_percentage_90(0,'2021-09-01');