CREATE OR REPLACE FUNCTION f_calcule_projection_percentage_70 (crs_id  INT DEFAULT 0, date_projection date DEFAULT '1900-01-01')  RETURNS SETOF  projection_percentage
	AS $BODY$
	DECLARE 
...
_query= 'SELECT * FROM detention_all'; 
	FOR _date_inq, _yearsq, _monthsq, _daysq, _prontuarioq,_nameppl,_last_nameppl, _number_dtq, _name_crsq,_crs_id   IN EXECUTE _query LOOP
			...
			END LOOP;	
	RETURN QUERY	 
 SELECT * FROM projection_percentage WHERE porcent_cumplido BETWEEN 70 AND 79.99;
...
SELECT f_calcule_projection_percentage_70(0,'2021-11-01');