CREATE OR REPLACE FUNCTION f_history_kardex (ide_geani  INT DEFAULT 0, ide_bocam  INT DEFAULT 0  )
RETURNS SETOF  kardex
AS $BODY$
DECLARE 
   _ide_geani INT;
   _ide_bocam INT;
...
BEGIN
_ide_geani=ide_geani;
_ide_bocam= ide_bocam;

WITH param AS (
    SELECT
   _ide_geani as ide_geani,
   _ide_bocam as ide_bocam
),
inicial AS (
SELECT
 ide_geani,
